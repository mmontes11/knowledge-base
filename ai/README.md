...
Projects in the AI / local-LLM-runtime domain.
| [ollama](ollama/) | Fast, local runtime for Open LLMs: runs models from the ollama.com library and user-created models on a single server, exposed through a JSON REST API (port `11434`), an OpenAI-compatible `/v1` API, and the `ollama` CLI; inference built on llama.cpp (CPU, CUDA, ROCm) plus MLX-based engines for Apple silicon. |
