...
- [`go/vt/mysqlctl/backup.go`](https://github.com/vitessio/vitess/blob/main/go/vt/mysqlctl/backup.go) and [`go/vt/mysqlctl/backupengine.go`](https://github.com/vitessio/vitess/blob/main/go/vt/mysqlctl/backupengine.go) — the backup/restore engine with pluggable backup tools (built-in, XtraBackup, MySQL Shell dump) and pluggable storage (`filebackupstorage/`, `gcsbackupstorage/`, `s3backupstorage/`, `cephbackupstorage/`, `azblobbackupstorage/` under the same package) — see the [Backup and restore](https://vitess.io/docs/24.0/user-guides/operating-vitess/backup-and-restore/) guide.
...
The implementation: VTGate opens one connection per shard for a transaction (`go/vt/vtgate/tx_conn.go`); in `TWOPC` mode a dedicated 2PC coordinator in [`go/vt/vtgate/txresolver/tx_resolver.go`](https://github.com/vitessio/vitess/blob/main/go/vt/vtgate/txresolver/tx_resolver.go) drives commit across shards, and each tablet participates as a 2PC participant via [`go/vt/vttablet/tabletserver/twopc.go`](https://github.com/vitessio/vitess/blob/main/go/vt/vttablet/tabletserver/twopc.go).
...
- [`go/vt/vtgate/`](https://github.com/vitessio/vitess/tree/main/go/vt/vtgate) — the transparent routing layer that hides sharding from clients: `vtgate.go` (gateway), `executor.go` (query execution), `tabletgateway.go` (tablet discovery across watched cells), and `vstream_manager.go` (binlog streaming so reads/CDC scale horizontally too).
...
- [`go/vt/mysqlctl/`](https://github.com/vitessio/vitess/tree/main/go/vt/mysqlctl) — the durability/backup machinery: `reparent.go`, `backup.go`/`backupengine.go`, plus the storage backends.
